...
import '../../riskProfile/media/css/main.css' 
...
export default function Accodion() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/riskProfile/getRiskProfileQuestions",)
      .then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (axiosCall) {
      ...
      axios
        .post("http://localhost:3000/riskProfile/insertProfileData", { obj, name, email, mobile })
        .then((res) => {
          !res.data.status &&
            axios
              .get("http://localhost:3000/riskProfile/getGraphData", { params: { obj, name, email, mobile } })
              .then((res) => {
    ...
    <div id='riskProfile'>
      <section className={`outerContainer ${(openPopupForm) && "blurBackground"} ${!riskMeter.status && "blurRiskGraphBackgroundGraph"}`}>
        ...
        <MyAccodian
          ...
          obj={obj}
        />
        <button className="btn prevBtn" disabled={currentPage && currentPage == 1} onClick={() => { (currentPage != 1) && setCurrPage(currentPage - 1) }}>Prev</button>
